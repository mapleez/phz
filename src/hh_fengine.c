...
{
	if(ntHeader->_nt_header==NULL)
	{
		DWORD dwRead;
	ntHeader->_nt_header=(IMAGE_NT_HEADERS *)malloc(sizeof(IMAGE_NT_HEADERS));
	//DWORD dwRead;
	if(dosHeader->_dos_header.e_magic==IMAGE_DOS_SINGATURE)  //是不是有效的DOS头
		{   //定位NT头
			...
			{  //读取NT头
				ReadFile(hFile,&ntHeader->_nt_header,sizeof(ntHeader->_nt_header),NULL,NULL)
					if(dwRead==sizeof(ntHeader->_nt_header))
				{
					return dwRead;
				}
			}
	...
	return sizeof(ntHeader->_nt_header);
}
